...
func TokenValid(r *http.Request) error {
	...
	logging.Info(logTag, tokenString)

	token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {

		flag := token.Method.(*jwt.SigningMethodHMAC)
		fmt.Println(flag)
		...
		logging.Info(logTag, config.Config.ApiSecret)
		return []byte(config.Config.ApiSecret), nil
	})
	if err != nil {
		logging.Error(logTag, "Invalid authentication token", err)
		return err
	}
